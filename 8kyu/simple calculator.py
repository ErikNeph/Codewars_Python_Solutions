...
Note: In dynamically typed languages (JS, PHP, Python), the first and second arguments can be not numbers.
In that case, return "unknown value".
...
def calculator(x: int, y: int, op: str) -> int | float | str:
    if (type(x) == int and type(y) == int) and (type(op) == str and op in "+-*/" and len(op) == 1):
