...
His parents can't keep up calculating this so they've begged you to help them out by writing
a programme that can answer Philip's endless questions.

Your task is to write a function that takes two parameters:
the year of birth and the year to count years in relation to. As Philip is getting
more curious every day he may soon want to know how many years it was until he would be born,
...
def calculate_age(year_of_birth, current_year):
    curr_age = current_year - year_of_birth
    year_or_years = "year" if curr_age == 1 or curr_age == -1 else "years"
    if year_of_birth == current_year:
        return "You were born this very year!"
    elif year_of_birth < current_year:
        return f"You are {curr_age} {year_or_years} old."
    else:
        return f"You will be born in {abs(curr_age)} {year_or_years}."
