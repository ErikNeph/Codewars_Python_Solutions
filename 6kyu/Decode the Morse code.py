...
# the character codes and 3 spaces are used to separate words.the.
#  For example, the message HEY JUDE in Morse code is ···· · −·−−   ·−−− ··− −·· ·.
# NOTE: Extra spaces before or after the code have no meaning and should be ignored.
# In addition to letters, digits and some punctuation,
# there are some special service codes, the most notorious of those
# is the international distress signal SOS (that was first issued by Titanic),
# that is coded as ···−−−···. These special codes are treated
...
def decode_morse(morse_code):
    return " ".join(
        "".join(MORSE_CODE[i] for i in word.split(" "))
